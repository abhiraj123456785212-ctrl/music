...
playlist_songsdb = mongodb.playlist_songs
# ✅ New: Quality settings collection
qualitydb = mongodb.quality_settings
# ✅ New: Buffer settings collection
bufferdb = mongodb.buffer_settings
...
# ✅ New: Get quality setting for a chat
async def get_quality(chat_id: int) -> str:
    """Get quality setting for a chat (default: medium)"""
    if chat_id in quality_cache:
        return quality_cache[chat_id]
    
    data = await qualitydb.find_one({"chat_id": chat_id})
    if not data:
        quality_cache[chat_id] = "medium"
        return "medium"
    
    quality_cache[chat_id] = data.get("quality", "medium")
    return quality_cache[chat_id]


# ✅ New: Set quality for a chat
async def set_quality(chat_id: int, quality: str) -> bool:
    """Set quality for a chat"""
    valid_qualities = ["low", "medium", "high", "ultra"]
    if quality not in valid_qualities:
        return False
    
    quality_cache[chat_id] = quality
    await qualitydb.update_one(
        {"chat_id": chat_id},
        {"$set": {"quality": quality}},
        upsert=True
    )
...
# ✅ New: Get default quality
async def get_default_quality() -> str:
    """Get global default quality"""
    data = await qualitydb.find_one({"chat_id": "global"})
    if not data:
        return "medium"
    return data.get("quality", "medium")


# ✅ New: Set default quality
async def set_default_quality(quality: str) -> bool:
...
# ✅ New: Get buffer setting for a chat
async def get_buffer_size(chat_id: int) -> int:
    """Get buffer size for a chat (default: 10 seconds)"""
    if chat_id in buffer_cache:
        return buffer_cache[chat_id]
    
    data = await bufferdb.find_one({"chat_id": chat_id})
    if not data:
        buffer_cache[chat_id] = 10
        return 10
    
    buffer_cache[chat_id] = data.get("buffer_size", 10)
    return buffer_cache[chat_id]


# ✅ New: Set buffer for a chat
async def set_buffer_size(chat_id: int, size: int) -> bool:
    """Set buffer size for a chat (3-30 seconds)"""
    if not 3 <= size <= 30:
        return False
    
    buffer_cache[chat_id] = size
    await bufferdb.update_one(
        {"chat_id": chat_id},
...
# ✅ New: Get default buffer size
async def get_default_buffer() -> int:
    """Get global default buffer size"""
    data = await bufferdb.find_one({"chat_id": "global"})
    if not data:
        return 10
    return data.get("buffer_size", 10)


# ✅ New: Set default buffer size
async def set_default_buffer(size: int) -> bool:
    """Set global default buffer size"""
    if not 3 <= size <= 30:
        return False
...
async def create_playlist(user_id: int, name: str, is_public: bool = False) -> bool:
    """Create a new playlist for a user"""
    # Check if playlist name already exists
    existing = await playlistdb.find_one({
        "user_id": user_id,
        "name": name
    })
    if existing:
    ...
    await playlistdb.insert_one(playlist_data)
    return True


# ✅ New: Delete a playlist
async def delete_playlist(user_id: int, name: str) -> bool:
    """Delete a playlist"""
    result = await playlistdb.delete_one({
        "user_id": user_id,
        "name": name
    })
    if result.deleted_count > 0:
        # Also delete all songs in this playlist
        await playlist_songsdb.delete_many({
            "user_id": user_id,
            "playlist_name": name
        })
        return True
...
async def add_song_to_playlist(
    ...
    existing = await playlist_songsdb.find_one({
        "user_id": user_id,
    ...
    await playlist_songsdb.insert_one(song_data)
    
    # Update total songs count
    await playlistdb.update_one(
        {"user_id": user_id, "name": playlist_name},
        {"$inc": {"total_songs": 1}, "$set": {"updated_at": datetime.now()}}
    )
    return True
...
async def remove_song_from_playlist(
    ...
    result = await playlist_songsdb.delete_one({
        "user_id": user_id,
        "playlist_name": playlist_name,
        "vidid": vidid
    })
...
async def get_playlist_songs(
    ...
    """Get all songs from a playlist with pagination"""
    songs = []
    cursor = playlist_songsdb.find({
        "user_id": user_id,
        "playlist_name": playlist_name
...
# ✅ New: Get public playlists
async def get_public_playlists(limit: int = 50) -> List[Dict]:
    """Get public playlists"""
    playlists = []
    cursor = playlistdb.find({"is_public": True}).limit(limit)
    async for playlist in cursor:
        playlists.append(playlist)
    return playlists


# ✅ New: Check if playlist exists
async def playlist_exists(user_id: int, name: str) -> bool:
...
async def rename_playlist(user_id: int, old_name: str, new_name: str) -> bool:
    ...
    if result.modified_count > 0:
        ...
        await playlist_songsdb.update_many(
            {"user_id": user_id, "playlist_name": old_name},
